...
class TestDataCleaner(unittest.TestCase):
    ...
    def test_drop_invalid_rows_removes_rows_with_missing_values(self):
        ...
        self.assertFalse(any(result["name"].isna()))
        self.assertFalse(any(result["age"].isna()))
    ...
    def test_trim_strings_raises_typeerror_for_non_string_column(self):
        ...
        # Lanza una excepción TypeError en columnas no string. 
    ...
    def test_remove_outliers_iqr_removes_extreme_values(self):
        ...
        df = make_sample_df()
        cleaner = DataCleaner()
        # el factor 1.5 no funciona, y debería ser 0.5 para eliminar 120 en este conjunto de datos pequeño
        # iqr es de 25+30 es 47.5, si el factor es 0.5, entonces el low bound es 
        # 30 - 0.5*47.5 = 6.25 y el high bound es 77.5 + 0.5*47.5 = 101.25, por lo que 120 es eliminado
        result = cleaner.remove_outliers_iqr(df, "age", factor=0.5)

        # el 120 no debería estar
        self.assertNotIn(120, result["age"].tolist())
        # el 25 o el 35 deberían estar
        self.assertIn(25, result["age"].tolist())
        self.assertIn(35, result["age"].tolist())
    ...
    def test_remove_outliers_iqr_raises_keyerror_for_missing_column(self):
        ...
        # testeamos que lanza KeyError en columnas no existentes
        with self.assertRaises(KeyError):
            cleaner.remove_outliers_iqr(df, "columna_no_existente")
    ...
    def test_remove_outliers_iqr_raises_typeerror_for_non_numeric_column(self):
        ...
        df = make_sample_df()
        df["city"] = df["city"].astype("string")  # Asegurar que es tipo string
        cleaner = DataCleaner()

        # testeamos que lanza TypeError en columnas no numéricas
        with self.assertRaises(TypeError):
