...
class TestStatisticsUtils(unittest.TestCase):
    ...
    def test_moving_average_only_accepts_1d_sequences(self):
        ...
        """
        mult_dim_seq = [[1, 2], [3, 4]]
        utils = StatisticsUtils()
        # Lanza una excepción ValueError para secuencias multidimensionales.
        with self.assertRaises(ValueError):
            utils.moving_average(mult_dim_seq, window=2)
    ...
    def test_zscore_has_mean_zero_and_unit_std(self):
        ...
        result = sutils.zscore(nums)

        # verificamos que la media es aproximadamente 0
        res_avg = np.mean(result)
        self.assertAlmostEqual(res_avg, 0.0, places=7)

        # verificamos que la desviación estándar es aproximadamente 1
        res_std = np.std(result) 
        self.assertAlmostEqual(res_std, 1.0, places=7)
    ...
    def test_min_max_scale_maps_to_zero_one_range(self):
        ...
        nums = [2, 4, 6, 8, 10]
        sutils = StatisticsUtils()
        result = sutils.min_max_scale(nums)

        # verificamos que el valor mínimo es 0.0
        val_min = np.min(result)
        self.assertAlmostEqual(val_min, 0.0, places=7)

        # verificamos que el valor máximo es 1.0
        val_max = np.max(result)
        self.assertAlmostEqual(val_max, 1.0, places=7)

        # verificamos que los valores transformados son correctos
    ...
    def test_min_max_scale_raises_for_constant_values(self):
        ...
        """
        constant_nums = [7, 7, 7, 7]
        sutils = StatisticsUtils()

        # Lanza una excepción ValueError para secuencias con todos los valores iguales.
        with self.assertRaisesRegex(ValueError, "All values are equal"):
            sutils.min_max_scale(constant_nums)
